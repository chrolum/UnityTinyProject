...
public class GameMap : MonoBehaviour {
    ...
    const int END = 9;
    const int WALL = 1;
    const int EMPTY = short.MaxValue;
    ...
    IEnumerator BFS()
    {
        map_search = new short[map.GetLength(0),map.GetLength(1)];
        ...
        Func func = (Pos cur, int ox, int oy) =>
        {
            ...
            {
                map_search[cur.y + oy, cur.x + ox] = (short)(map_search[cur.y, cur.x] + 1);
            ...
            {
                // 核心，最重要的判断
                if (map_search[cur.y + oy, cur.x + ox] > map_search[cur.y, cur.x] + 1)
                {
                    map_search[cur.y + oy, cur.x + ox] = (short)(map_search[cur.y, cur.x] + 1);
        ...
        map_search[startPos.y, startPos.x] = 0;
        ...
        while (queue.Count > 0)
        {
            ...
            if (map_search[cur.y, cur.x] > cur_depth)
            {
               cur_depth = map_search[cur.y, cur.x];
               RefreshPath(map_search);
    ...
    {
        Pos pos = endPos;
        short step = map_search[pos.y, pos.x];
        while (!pos.Equals(startPos))
        {
            if (pos.y > 0 && map_search[pos.y-1, pos.x]==step-1)
            {
                pos = new Pos(pos.x, pos.y - 1);
                step -= 1;
            }
            else if (pos.y < H - 1 && map_search[pos.y+1, pos.x]==step-1)
            {
                pos = new Pos(pos.x, pos.y + 1);
                step -= 1;
            }
            else if (pos.x > 0 && map_search[pos.y, pos.x-1]==step-1)
            {
                pos = new Pos(pos.x-1, pos.y);
                step -= 1;
            }
            else if (pos.x < W - 1 && map_search[pos.y, pos.x+1]==step-1)
            {
                pos = new Pos(pos.x+1, pos.y);
                step -= 1;
            }
    ...
    void RefreshPath(short[,] temp_map)
    {
        for (int i = 0; i < H; i++)
        {
            for (int j = 0; j < W; j++)
            {
                if (temp_map[i,j] == short.MaxValue)
                {
                    pathBlocks[i, j].SetActive(false);
                }
                else
                {
                    ...
                    {
                        texts[0].text = map_search[i, j].ToString();
                        texts[1].text = "";
                    }
                    else if (searchMethod == SearchMethod.AStar)
                    {
                        texts[0].text = astar_search[i, j].G.ToString();
                        texts[1].text = astar_search[i, j].H.ToString();
                    }
                }
            }
        }
